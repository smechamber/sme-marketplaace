...
const request = async (url, options = {}) => {
  ...
  if (!response.ok) { const error = new Error(body.error || "Request failed"); error.data = body; throw error }
  return body
}
...
function makeCollection(name) {
  return {
    ...
    async getFirstListItem(filter, opts = {}) { const data = await this.getList(1, 1, { ...opts, filter }); if (!data.items.length) throw new Error("Record not found"); return data.items[0] },
