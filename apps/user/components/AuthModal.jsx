...
export default function AuthModal({ open, onOpenChange, defaultMode = "login" }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (mode === "login") {
        ...
        onOpenChange(false)
        if (user?.userRole === "seller") window.location.href = process.env.NEXT_PUBLIC_SELLER_URL || "http://localhost:3001"
  ...
  const verifyOtp = async (e) => {
    ...
    try {
      ...
      onOpenChange(false)
      if (role === "seller") window.location.href = process.env.NEXT_PUBLIC_SELLER_URL || "http://localhost:3001"
      else router.push("/dashboard")
